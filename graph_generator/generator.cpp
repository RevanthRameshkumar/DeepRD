...
{
	array<pair<const node*, const node*>> frontier(16);
	for (const node* v : vertex->children)
		frontier.add(make_pair(v, v));
	hash_set<const node*> visited(64);
	visited.add_all(vertex->children.data, vertex->children.length);
...
void get_descendants(const node& vertex, hash_set<const node*>& descendants) {
	array<const node*> queue(16);
	hash_set<const node*> visited(64);
	queue[0] = &vertex;
	queue.length = 1;
...
bool has_cycles(array<node>& vertices) {
	for (const node& vertex : vertices) {
		hash_set<const node*> descendants(64);
...
bool generate_graph_with_lookahead(array<node>& vertices, node*& start, node*& end, unsigned int num_vertices, unsigned int max_num_parents, unsigned int max_vertex_id, unsigned int lookahead, unsigned int num_paths, unsigned int max_prefix_vertices, unsigned int max_edges)
{
	...
	for (unsigned int i = 0; i < index; i++) {
		total_edges += vertices[i].children.length;
	}

	array<float> degree_distribution(max_num_parents + 1);
	degree_distribution.length = max_num_parents + 1;
	constexpr float BETA = -1.0f;
	degree_distribution[0] = 1.0f;
	float total_degree_probability = 0.0f;
	for (unsigned int i = 0; i < degree_distribution.length; i++) {
		degree_distribution[i] = pow(i + 1, BETA);
		total_degree_probability += degree_distribution[i];
	}

	float expected_degree = 0.0f;
	for (unsigned int i = 0; i < degree_distribution.length; i++)
		expected_degree += i * degree_distribution[i] / total_degree_probability;
	float expected_nonzero_degree = 0.0f;
	for (unsigned int i = 1; i < degree_distribution.length; i++)
		expected_degree += i * degree_distribution[i] / (total_degree_probability - degree_distribution[0]);
	float expected_edges_per_vertex = expected_degree + expected_nonzero_degree*degree_distribution[0]/total_degree_probability + expected_degree*(1.0f - degree_distribution[0]/total_degree_probability);
	...
	hash_set<const node*> valid_path_vertices(16);
	get_descendants(vertices[1], valid_path_vertices);
unsigned int in_out_vertices = 0;
unsigned int total_vertices = 0;
	for (unsigned int i = index; i < num_vertices; i++) {
		/* sample the number of child and parent vertices */
		unsigned int num_children = sample_categorical(degree_distribution.data, total_degree_probability, degree_distribution.length);
		if (num_children == 0) {
			degree_distribution[0] = 0.0f;
			total_degree_probability -= 1.0f;
		}
		unsigned int num_parents = sample_categorical(degree_distribution.data, total_degree_probability, degree_distribution.length);
		if (num_children == 0) {
			degree_distribution[0] = 1.0f;
			total_degree_probability += 1.0f;
		}
		...
		/* check if we would add too many edges */
		if (total_edges + num_children + num_parents > max_edges)
			break;

		/* sample the children of this new node */
		array<float> probabilities(index + 1);
		...
		total_edges += sampled_children.length;

		/* check if any children are on a the path from the start vertex to the goal vertex */
		bool on_valid_path = false;
		for (unsigned int child_id : sampled_children) {
			if (valid_path_vertices.contains(&vertices[child_id])) {
				on_valid_path = true;
				break;
			}
		}
		...
		hash_set<const node*> descendants(16);
		if (on_valid_path) {
			/* if the new vertex has children on a path to the goal, we can only sample parents that are not descendants of the start vertex */
			get_descendants(vertices[0], descendants);
		...
		if (total_probability != 0.0f) {
			num_parents = std::min(num_parents, index - (unsigned int) descendants.size);
			...
			total_edges += sampled_parents.length;
if (sampled_parents.length != 0 && sampled_children.length != 0) {
in_out_vertices++;
}
//fprintf(stderr, "%u %u (%f)\n", sampled_children.length, sampled_parents.length, (float) in_out_vertices / total_vertices);

			/* if the new vertex has a child and a parent on a valid path, then this vertex is also on a valid path */
			if (on_valid_path) {
				for (unsigned int parent_id : sampled_parents) {
					if (valid_path_vertices.contains(&vertices[parent_id])) {
						valid_path_vertices.add(&vertices[index]);
						break;
					}
				}
			}

			/* check to make sure the lookahead isn't decreased by this new vertex */
			if (lookahead_depth(start, &vertices[1], end) != lookahead) {
				/* the lookahead has changed, so remove the parents */
				if (sampled_children.length != 0 && randrange(2) == 1) {
					for (unsigned int parent_id : sampled_parents) {
						vertices[parent_id].children.remove(vertices[parent_id].children.index_of(&vertices[index]));
						vertices[index].parents.remove(vertices[index].parents.index_of(&vertices[parent_id]));
						out_degrees[parent_id] -= 1;
					}
					total_edges -= sampled_parents.length;
				} else {
					for (unsigned int child_id : sampled_children) {
						vertices[index].children.remove(vertices[index].children.index_of(&vertices[child_id]));
						vertices[child_id].parents.remove(vertices[child_id].parents.index_of(&vertices[index]));
						in_degrees[child_id] -= 1;
					}
					total_edges -= sampled_children.length;
				}
...
bool generate_example(array<node>& vertices, node*& start, node*& end, array<array<node*>>& paths, unsigned int num_vertices, unsigned int max_num_parents, unsigned int max_vertex_id, bool get_shortest_paths, int lookahead, unsigned int num_paths, unsigned int max_prefix_vertices, unsigned int max_edges)
{
	if (lookahead == -1) {
		if (!generate_graph(vertices, start, end, num_vertices, max_num_parents, max_vertex_id))
			return false;
	} else {
		if (!generate_graph_with_lookahead(vertices, start, end, num_vertices, max_num_parents, max_vertex_id, lookahead, num_paths, max_prefix_vertices, max_edges))
			return false;
	}
...
py::tuple generate_training_set(const unsigned int max_input_size, const uint64_t dataset_size, const int max_lookahead,
const unsigned int max_edges, const py::object& reserved_inputs, const int distance_from_start, const int max_prefix_vertices, const int num_branches, const bool quiet=false)
{
	...
	if (max_lookahead == -1) {
		...
		MAX_FREQS_PER_BUCKET[max_lookahead] = 1.05;
	}
	...
	while (num_generated < dataset_size) {
		...
		while (true) {
			if (max_lookahead == -1) {
				...
				if (!generate_example(g, start, end, paths, num_vertices, 4, (max_input_size - 5) / 3, true, -1, 0, max_prefix_vertices == -1 ? max_input_size : max_prefix_vertices, max_edges)) {
				...
				unsigned int num_vertices = std::min(std::min(lookahead * num_branches + 1 + randrange(0, max_input_size/3), (max_input_size-5) / 3), max_edges + 1);
				if (!generate_example(g, start, end, paths, num_vertices, 4, (max_input_size - 5) / 3, true, lookahead, num_branches, max_prefix_vertices == -1 ? max_input_size : max_prefix_vertices, max_edges)) {
					for (node& n : g) core::free(n);
					for (array<node*>& a : paths) core::free(a);
					g.length = 0; paths.length = 0;
					continue;
		...
		for (const array<node*>& path : paths) {
			...
			for (unsigned int j = 1; j < path.length; j++) {
				...
				if (max_lookahead != 1 && lookahead_steps != max_lookahead)
					continue;
				if (useful_steps.length != 1)
					continue;
...
py::array_t<int64_t, py::array::c_style> lookahead_histogram(const unsigned int max_input_size, const uint64_t num_samples, const unsigned int max_edges, const int distance_from_start, const bool quiet=false)
{
	...
	while (num_generated < num_samples) {
		...
		while (true) {
			unsigned int num_vertices = randrange(3, (max_input_size - 5) / 3);
			if (!generate_example(g, start, end, paths, num_vertices, 4, (max_input_size - 5) / 3, true, -1, 0, -1, max_edges)) {
				for (node& n : g) core::free(n);
				for (array<node*>& a : paths) core::free(a);
				g.length = 0; paths.length = 0;
				continue;
...
py::tuple generate_reachable_training_set(const unsigned int max_input_size, const uint64_t dataset_size, const unsigned int lookahead, const unsigned int max_edges, const py::object& reserved_inputs, const int distance_from_start, const int reachable_distance, const unsigned int start_vertex_index, const bool exclude_start_vertex)
{
	...
	while (num_generated < dataset_size) {
		...
		while (true) {
			...
			if (!generate_example(g, start, end, paths, num_vertices, 4, max_vertex_id, true, lookahead, num_paths, -1, max_edges)) {
				for (node& n : g) core::free(n);
				for (array<node*>& a : paths) core::free(a);
				g.length = 0; paths.length = 0;
				continue;
